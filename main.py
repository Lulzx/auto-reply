...
config, time, messagefilters, functions client = 
TelegramClient(config.session_id, config.api_id, 
config.api_hash, update_workers=4) client.connect() 
if not client.is_user_authorized():
    client.sign_in(config.phone)
    client.sign_in(code=input("Code:")) 
print(len(client.get_dialogs())) def 
printupdate(update):
    print(update)
    if messagefilters.Filters.command(update) and 
isinstance(update, UpdateShortMessage):
